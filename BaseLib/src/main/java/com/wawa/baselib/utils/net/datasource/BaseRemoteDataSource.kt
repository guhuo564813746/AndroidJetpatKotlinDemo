...
import com.wawa.baselib.utils.net.callback.BaseRequestCallback
...
import com.wawa.baselib.utils.net.exception.LocalBadException
...
/**
 *作者：create by 张金 on 2021/1/18 11:13
 *邮箱：564813746@qq.com
 */
abstract class BaseRemoteDataSource<Api : Any>(protected val iUIActionEvent:IUIActionEvent?, protected val apiServiceClass: Class<Api>) : CoroutineEvent{
    companion object{
        /**
         * ApiService 缓存
         */
     ...
     * 和生命周期绑定的协程作用域
     */
    override val lifecycleSuportScope=iUIActionEvent?.lifecycleSuportScope ?: GlobalScope
    ...
    fun getApiService(baseUrl: String,apiServiceClass: Class<Api>): Api{
    ...
    internal fun generateExceptionReal(throwable: Throwable): BaseHttpException{
    ...
    protected open fun exceptionFormat(httpException: BaseHttpException): String{
        return when(httpException.realException){
            null ->{
                httpException.err_msg
            }
            is ConnectException,is SocketTimeoutException,is UnknownHostException ->{
                "连接超时，请检查您的网络设置"
            }
            else->{
                "请求过程抛出异常:"+httpException.err_msg
            }
        }
    }
